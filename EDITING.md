...
| **某个服务器(分类/名称/区域/模式/人数/延迟/在线/主页/接入地址)** | `src/content/servers/<id>.yaml`(一台一个文件;`type` 为分类,见下行;`online:false` 显示离线;`address: 'ip:port'` 是接入地址(卡片显示并生成 CM 一键加入),`homepage: 'https://…'` 是介绍页主页) |
