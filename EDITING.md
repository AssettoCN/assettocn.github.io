...
| **某个服务器(分类/名称/区域/模式/容量/在线/主页/接入地址)** | `src/content/servers/<id>.yaml`(一台一个文件;`type` 为分类,见下行;`max` 仅作静态"最多 N 人"展示,不再有实时人数/延迟;`online:false` 显示离线;`address: 'ip:port'` 是接入地址(卡片显示并生成 CM 一键加入),`homepage: 'https://…'` 是介绍页主页) |
